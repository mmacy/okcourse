...
app = FastAPI(
    title="OKCourse API", description="Generate audiobook-style courses with lectures on any topic", version="0.1.0"
)
...
async def create_course(request: CourseCreateRequest):
    ...
    try:
        ...
        return {"course_id": course_id, "title": course.title, "status": "created"}
    except Exception as e:
...
async def generate_outline(course_id: str):
    ...
    try:
        ...
        return {"status": "completed", "outline": course.outline.model_dump() if course.outline else None}
    except Exception as e:
...
async def generate_lectures(course_id: str):
    ...
    try:
        ...
        return {"status": "completed", "lectures": [lecture.model_dump() for lecture in course.lectures]}
    except Exception as e:
        ...
        while True:
            try:
                status_data = {
                    ...
                    "has_image": (
                        course.generation_info.image_file_path is not None
                        and course.generation_info.image_file_path.exists()
                    )
                    if course.generation_info
                    else False,
                    "has_audio": (
                        course.generation_info.audio_file_path is not None
                        and course.generation_info.audio_file_path.exists()
                    )
                    if course.generation_info
                    else False,
                    "generation_info": course.generation_info.model_dump() if course.generation_info else None,
...
@app.get("/api/courses/{course_id}/image")
async def serve_course_image(course_id: str):
    """Serve the generated course image file."""
    if course_id not in active_courses:
        raise HTTPException(status_code=404, detail="Course not found")

    course = active_courses[course_id]
    if not course.generation_info or not course.generation_info.image_file_path:
    ...
    return FileResponse(path=str(image_path), media_type="image/png", filename=f"{course.title}_cover.png")


@app.get("/api/courses/{course_id}/audio")
async def serve_course_audio(course_id: str):
    """Serve the generated course audio file."""
    if course_id not in active_courses:
        raise HTTPException(status_code=404, detail="Course not found")

    course = active_courses[course_id]
    if not course.generation_info or not course.generation_info.audio_file_path:
    ...
    return FileResponse(path=str(audio_path), media_type="audio/mpeg", filename=f"{course.title}_course.mp3")
