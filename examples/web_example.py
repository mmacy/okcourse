...
from okcourse import NUM_LECTURES, format_time, generate_course

st.title("Lecture Series Generator")

topic = st.text_input("Enter course topic", "")
...
if st.button("Generate course"):
    ...
    with st.spinner("Generating lectures..."):
        results = generate_course(topic.strip(), num_lectures, generate_audio_file)
