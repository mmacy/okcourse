...
async def generate_course_image_async(
    ...
    Returns:
        A tuple of PNG image in bytes and the path to the image file if `image_file_path` is provided.
        If generation fails, returns (None, None).
    """
    try:
...
def generate_course_image(course_outline: CourseOutline, image_file_path: Path) -> tuple[bytes | None, Path | None]:
