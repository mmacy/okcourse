...
async def write_course_to_file_async(course: Course, output_dir: Path) -> Path:
    ...
    async with aiofiles.open(course_text_path, "w", encoding="utf-8") as f:
        await f.write(course.model_dump_json())
...
async def generate_course_image_async(course_outline: CourseOutline, image_file_path: Path) -> Path | None:
...
async def generate_course_audio_async(
    ...
    if do_generate_cover_image:
        ...
        cover_image_file = await generate_course_image_async(
...
async def generate_course_async(
    ...
    Args:
        ...
        do_generate_cover_art: Whether to generate a cover art image for the course and MP3, if generated.
    ...
    course_outline = await generate_course_outline_async(topic, num_lectures)
    ...
    course = await generate_course_lectures_async(course_outline)
    ...
    course_text_path = await write_course_to_file_async(course, output_dir)
    ...
    if do_generate_audio_file:
        ...
        mp3_path = await generate_course_audio_async(course, str(mp3_path), do_generate_cover_art)
...
def write_course_to_file(course: Course, output_dir: Path) -> Path:
    """Synchronous wrapper for write_course_to_file_async."""
    return asyncio.run(write_course_to_file_async(course, output_dir))
...
def generate_course(
    ...
    """Synchronous wrapper for generate_course_async."""
    return asyncio.run(generate_course_async(topic, num_lectures, do_generate_audio_file, do_generate_cover_art))
